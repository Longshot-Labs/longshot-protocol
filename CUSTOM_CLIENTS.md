...
The API's current wallet policy is authoritative in
[`docs/src/api/auth.md`](https://github.com/Longshot-Labs/longshot/blob/main/docs/src/api/auth.md);
signed-RFQ and withdrawal policy is authoritative in
[`docs/src/api/trading.md`](https://github.com/Longshot-Labs/longshot/blob/main/docs/src/api/trading.md).
...
signature, timestamp, and optional access codes.
...
2. Supply odds to the binary type as integer basis points:
   ...
   The HTTP DTO uses `min_odds: 2.5`. Supported HTTP odds are greater than
   `1.0x` and no greater than the protocol `Odds.MAX` value.
...
2. Choose the exact `amount_micros`, destination address, and a canonical UUID
   `idempotency_key`. An omitted destination defaults to the signing EOA, so use
   that address while building the message.
...
The checked examples use fixed test keys, timestamps, and sample deployment
values, and make no network calls:

- `typescript/examples/custom-client.ts`
- `python/examples/custom_client.py`

They are compiled or imported by the package test suites. Their generated
requests are deterministic test vectors, not live submission payloads. Never
reuse their private keys outside tests; inject the target deployment's domain,
chain ID, and current timestamps in a real client.

Run the protocol suites from the repository root:
